...
# TOOD: address mid_ch in ghost block
class DeRain_v2(nn.Module):
    def __init__(self, ghost=1, mix=False):
        ...
        self.ghost_block1 = GhostBottleneck(16, self.ghost * 16, 16)
        ...
        self.ghost_block2 = GhostBottleneck(32, self.ghost * 32, 32)
        ...
        self.ghost_block3 = GhostBottleneck(32, self.ghost * 32, 32)
        ...
        self.ghost_block4 = GhostBottleneck(128, self.ghost * 128, 128)
        ...
        self.ghost_block5 = GhostBottleneck(128, self.ghost * 128, 128)
        ...
        self.ghost_block6 = GhostBottleneck(128, self.ghost * 128, 128)
        ...
        self.ghost_block7 = GhostBottleneck(128, self.ghost * 128, 128)
        ...
        self.ghost_block8 = GhostBottleneck(64, self.ghost * 64, 64)
        ...
        self.ghost_block9 = GhostBottleneck(80, self.ghost * 80, 80)
    ...
    def forward(self, xin):
        ...
        # Size - 1.0
        # x1 = (self.dense_block1(x)) if not self.ghost else self.ghost_block1(x)
        x1 = self.ghost_block1(x)

        # Size - 0.5
        x2_i = self.trans_block2(x1)
        # x2_i = self.dense_block2(x2_i) if not self.ghost else self.ghost_block2(x2_i)
        x2_i = self.ghost_block2(x2_i)
        x2 = self.trans_block2_o(x2_i)

        # Size - 0.25
        x3_i = self.trans_block3(x2)
        # x3_i = self.dense_block3(x3_i) if not self.ghost else self.ghost_block3(x3_i)
        x3_i = self.ghost_block3(x3_i)
        x3 = self.trans_block3_o(x3_i)

        # Size - 0.125
        x4_i = self.trans_block4(x3)
        # x4_i = self.dense_block4(x4_i) if not self.ghost else self.ghost_block4(x4_i)
        x4_i = self.ghost_block4(x4_i)
        x4 = self.trans_block4_o(x4_i)

        # x5_i = self.dense_block5(x4) if not self.ghost else self.ghost_block5(x4)
        x5_i = self.ghost_block5(x4)
        x5 = self.trans_block5_o(x5_i)

        # x6_i = self.dense_block6(x5) if not self.ghost else self.ghost_block6(x5)
        x6_i = self.ghost_block6(x5)
        x6 = self.trans_block6_o(x6_i)
        z = self.zout(self.relu(x6))

        x7_i = self.trans_block7(z)  # TODO: mix HERE
        # print(x4.size())
        # print(x7_i.size())
        # x7_i = self.dense_block7(torch.cat([x7_i, x3], 1)) if not self.ghost else self.ghost_block7(torch.cat([x7_i, x3], 1))
        x7_i = self.ghost_block7(torch.cat([x7_i, x3], 1))
        x7 = self.trans_block7_o(x7_i)

        x8_i = self.trans_block8(x7)  # TODO: mix HERE
        # x8_i = self.dense_block8(torch.cat([x8_i, x2], 1)) if not self.ghost else self.ghost_block8(torch.cat([x8_i, x2], 1))
        x8_i = self.ghost_block8(torch.cat([x8_i, x2], 1))
        x8 = self.trans_block8_o(x8_i)

        x9_i = self.trans_block9(x8)  # TODO: mix HERE
        # x9_i = self.dense_block9(torch.cat([x9_i, x1, x], 1)) if not self.ghost else self.ghost_block9(torch.cat([x9_i, x1, x], 1))
        x9_i = self.ghost_block9(torch.cat([x9_i, x1, x], 1))
